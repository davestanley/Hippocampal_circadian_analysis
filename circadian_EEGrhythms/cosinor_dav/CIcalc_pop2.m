function [CI_Amp_min, CI_Amp_max, CI_phi_min, CI_phi_max] = CIcalc_pop2(M, beta, gamma, alpha,colour)



% Gives same results as CIcalc_pop, but requires only M, beta, and gamma as
% inputs
...
%% Calculate population values based on individual values

    Mbar = mean(M,2);
    Bbar = mean(beta,2);
    Gbar = mean(gamma,2);
    k = size(beta,2);
    s2beta = 1/(k-1) * sum( ( beta - repmat(Bbar,[1 k 1]) ).^2,2);
    s2gamma = 1/(k-1) * sum( ( gamma - repmat(Gbar,[1 k 1]) ).^2,2);
    sbetagamma = 1/(k-1) * sum( (gamma - repmat(Gbar,[1 k 1])) .* (beta - repmat(Bbar,[1 k 1])),2 );
    
    [Ampbar phibar] = getamptheta(Bbar, Gbar);
    Amp = Ampbar;
    phi = phibar;
...
if plot_on
    set(gcf,'name','Rhythm Parameter Estimates with Joint Confidence Region', 'position', [245 357 643 600]);
