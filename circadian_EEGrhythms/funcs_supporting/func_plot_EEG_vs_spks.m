...
function func_plot_EEG_vs_spks(r_seiz, prepostchronic,smoothmode_ts,reload_spks);
        %prepostchronic = 1;     % 0 for all; 1 for pre; 2 for latency; 3 for chronic
        plottest_timeseries = 0;

        i=1;[T{i} X{i}] = extract_ts(r_seiz{i}, prepostchronic); 
        i=2;[T{i} X{i}] = extract_ts(r_seiz{i}, prepostchronic); 
        i=3;[T{i} X{i}] = extract_ts(r_seiz{i}, prepostchronic); 
        i=4;[T{i} X{i}] = extract_ts(r_seiz{i}, prepostchronic); 
...
%                 TR{i} = T{i}(index,chosen_band);
                ...
                if plottest_timeseries
                    figure;
                    plot(TR{i},XS{i});hold on; plot(TR{i},XR{i},'r')
